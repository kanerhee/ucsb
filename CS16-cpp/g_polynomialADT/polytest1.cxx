...
void test_root(const polynomial& test);
// Postcondition:  The user has been prompted for the parameters of find_root.
// The answer from find_root is then written to cout.
  ...
  cout << "have all been initialized." << endl;
    ...
    do
	  ...
	  case 'N':  test_np(p[current_index]);
	    break;
	  case 'R':  test_root(p[current_index]);
	    break;
	  case '+':
	    cout << "A + B: ";
	    view(p[0] + p[1]);
...
void print_menu()
{
  ...
  cout << "R - use the test_root function" << endl;
  cout << "+ - view A + B" << endl;
  cout << "- - view A - B" << endl;
  cout << "* - view A * B" << endl;
  cout << "  -   -   -   -   -   -   -   -   -   -   -   -" << endl;
...
void test_root(const polynomial& test)
{
  ...
  double guess;
  unsigned int maximum_it;
  double epsilon;

  cout << "Enter the initial guess: ";
  cin >> guess;
  cout << "Enter the maximum iterations: ";
  cin >> maximum_it;
  cout << "Enter epsilon: ";
  cin >> epsilon;
  test.find_root(answer, success, iterations, guess, maximum_it, epsilon);
  if (success)
    cout << "Root found: " << answer;
  else
    cout << "No root found";
